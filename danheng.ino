  ...
  void loop() {
    ...
    powerCheck();
      ...
      if ((calcState == LOW && lastCalcState == HIGH) && (mode == 0)) {
        mode += 1;
        clearScreen();
        delay(100);
      }

      if ((calcState == LOW && lastCalcState == HIGH) && (mode == 3)) {
        mode += 1;
        clearScreen();
        if(isOptimistic) {
          odds = cumulative(pulls);
        } else {
          odds = pessimistic(pulls);
        }
        delay(100);
      }
      ...
      if ((calcState == LOW && lastCalcState == HIGH) && (mode == 5)) {
        mode = 0;
        pulls = 0;
        odds = 0;
        isOptimistic = false;
      ...
      if (mode == 0) {
        ...
        if(digitalRead(pessPin) == LOW) {
          ...
          if(odds == 1.0 && ((isOptimistic && pulls == 90) || (!isOptimistic && pulls == 180))) {
            lcd.print(100.000);
          } else if (odds * 100 >= 99.999) {
            lcd.print(99.999);
          } else {
